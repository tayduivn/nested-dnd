...
var getFaIcons = new Promise((resolve)=>{
	https.get('https://raw.githubusercontent.com/FortAwesome/Font-Awesome/5.0.13/advanced-options/metadata/icons.json', 
	function(res) {
		...
		res.pipe(concat({ encoding: 'string' }, function(remoteSrc) {
			...
			for(var name in faIcons){
				...
				if(styles.includes('brands'))
					values.push('fab fa-'+name);
				if(styles.includes('regular'))
					values.push('far fa-'+name);
				if(styles.includes('solid'))
					values.push('fas fa-'+name);
