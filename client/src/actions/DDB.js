...
function ddbConvert(data) {
	...
	if (!bg.hasCustomBackground && bg.definition.featureName) {
		c.features.push({
			...
			desc: abbreviate
				? getShortDesc(bg.definition.featureName)
				: bg.definition.featureDescription
		});
	}
...
function getSpellCard({ definition: def, alwaysPrepared }, spellcasting) {
	const lvl = def.level;
	const shortRange =
		def.range && def.range.rangeValue > 5 ? def.range.rangeValue : undefined;
	...
	return {
		...
		castTime:
			def.castingTime &&
			def.castingTime.castingTimeInterval +
				" " +
				def.castingTime.castingTimeUnit,
		properties: null,
...
function processSpellcasting(description, c, clas, classData) {
	...
	const classConfig = configData.classConfigurations.find(
		c => c.name === clas.name
	);
...
function feature(
	...
	if (definition.dynamicModifiers) {
		name = definition.dynamicModifiers
	...
	getAdvResist(
		definition.parentFeature || result.name,
		c.advResist,
		definition.name
	);
...
function processMod(m, fName, c, clas, classData) {
	...
	switch (m.type) {
		case "proficiency":
			...
			break;
		case "language":
			var lang = isHarryPotter
				? harryPotterify(m.friendlySubtypeName)
...
function getAdvResist(name, { advantages, resistances, other }, childFeature) {
	switch (name) {
		...
		case "Fey Ancestry":
			advantages.push(
				"Saves against being charmed, Magic can't put you to sleep"
			);
			break;
		case "Gnome Cunning":
			advantages.push(
