...
const ItemFront = ({ item }) => (
	<div
		className={
			"card weapon" + (item.consumable ? " red" : item.uses ? " purple" : "")
		}
	>
...
const RollValues = ({
	...
	<div className="roll-values pin-bottom">
		...
		{attackType ? (
			<WriteIn label="Attack" dmgType={attackType} writein={true} />
		) : null}
		{saveData ? (
			<WriteIn label="Save" save={saveData.throw} writein={false} />
		) : null}
		...
		damage.progression &&
		level !== 0 &&
		typeof damage.progression === "string" ? (
			<WriteIn
				className="higherLevelDmg"
