...
const Desc = ({
	...
	<div className="roll-values pin-bottom">
		...
		{save && save.throw ? (
			<WriteIn label="Save" save={save.throw} writein={false} />
		) : (
			""
		)}
		{save.success ? <div className="save-success">{save.success}</div> : null}
		{damage.attack ? (
			<WriteIn label="Attack" dmgType={damage.attack} writein={true} />
		) : null}
		{damage.diceString ? (
			<WriteIn
...
function getClassName(level, castTime, uses, consumable) {
	var className =
		parseInt(level, 10) > 0 || (uses && uses.count) ? " purple" : "";
	className +=
...
const SpellFront = ({
	...
	<div
		className={
			"card spell " +
			getClassName(level, rest.castTime, rest.uses, rest.consumable)
		}
	>
