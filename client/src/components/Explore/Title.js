						...
						? this.props.favorites
								.filter(
									f =>
										...
										f.index !== this.props.index
								)
								.map(f => (
									<a key={f.index} href={"#" + f.index}>
										<li>{f.name}</li>
									</a>
								))
						: null}
		...
		if (this.state.focusNew) {
			if (this.newLine && this.newLine.current) {
				if (this.state.focusNew !== true) {
					var child = this.newLine.current.parentElement.children[
						this.state.focusNew
					];
			...
			>
				{desc.map((d, i) => (
					<li
						...
						ref={i === desc.length - 1 ? this.newLine : undefined}
		...
		return (
			<button id="bgOptions" tabIndex="0" ref={this.ref}>
				...
				<div
					id="bgOptionsPopover"
					className={
						"popover animated fadeIn " + (this.state.display ? "" : "d-none")
					}
					role="tooltip"
					onClick={this.handleClick}
				>
					...
					<div className="popover-body">
						{this.currentSelected() ? (
							<button id="clearColor" className="btn btn-default" data-class="">
...
const StyleOptions = ({
	...
	<div id="styleOptions" className="col-xs-auto right">
		...
		<button
			className={"btn " + cssClass}
			onClick={() => handleChange(index, "isFavorite", !isFavorite)}
		>
		...
		<div className="dropdown">
			...
			<div
				className={"dropdown-menu dropdown-menu-right " + cssClass}
			...
			>
				...
				<Link
					to={
						isa ? `${location}/${isa}/edit` : `${location}/create?isa=${name}`
					}
				>
					<button className={"btn dropdown-item " + cssClass}>
						{isa ? (
							<span>edit {isa} generator</span>
						) : (
							<span>create {name} generator</span>
						)}
					</button>
				</Link>
...
const Title = ({
	...
	<div id="title" className="col-lg">
		<div className="name mt-3 mb-2 row animated fadeIn no-gutters">
			<div
				className={
					"icon-wrap col-auto col-lg-12 px-0 mb-lg-1 " +
					(isUniverse ? "universe " : " ") +
					CENTER_ALIGN
				}
			>
				{isUniverse ? (
					<button
						className={
							"btn btn-outline " + cssClass + " " + (!icon ? "addNew" : "")
						}
						data-toggle="modal"
			...
			<div
				className={
					"col col-lg-12 px-2 justify-content-lg-center " + VERTICAL_ALIGN
				}
			>
				<div id="name" style={{ color: txt }}>
					<h1
						className="webfont"
						dangerouslySetInnerHTML={{ __html: name ? name : isa }}
						contentEditable={isUniverse}
		...
		<nav className={`buttons-bar row no-gutters`}>
			...
			{!isUniverse ? (
				...
				<StyleOptions
					{...rest}
					cssClass={savedCssClass || cssClass}
					txt={savedTxt || txt}
					...
					parentBG={
						...
						up[0] &&
						up[0].cssClass &&
						up[0].cssClass.split(" ").find(c => c.startsWith("bg-"))
					}
		...
		{isUniverse ? <SearchBar {...{ favorites, index }} /> : null}
	</div>
);
