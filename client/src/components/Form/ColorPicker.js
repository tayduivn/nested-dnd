				...
				<div key={i} className="row no-gutters" >
					{
						Object.keys(colors[name]).map((variant,j)=>(
							<div key={j} className={"col sample "+(cn = "bg-"+name+"-"+variant)+(colors[name][variant].toUpperCase() === (hex && hex.toUpperCase()) ? ' selected':'')} data-class={cn} data-hex={colors[name][variant]}></div>
						))
					}
					{ Array(15-Object.keys(colors[name]).length).fill(null).map((h,k)=><div key={k} className="col sample">&nbsp;</div>) }
				</div>
			)
		)
	}
...
class HexColorPicker extends React.Component {
	...
	render() {
		...
		return (
			<a id="txtOptions" tabIndex="0" ref={this.ref}>
				<button className={"btn txt "+this.props.cssClass} onClick={ this.handleOpen } type="button" >
					<div className={`txt swatch`}></div>	
				</button>
				{ this.state.display ? 
				<div className="popover animated fadeIn"  role="tooltip" style={ popover } onClick={ this.handleClick }>
					<div className="arrow"></div>
					<div className="popover-body form-inline">
						<input id="txtInput" className="form-control mb-1" value={this.state.color} 
							onChange={debounce(this.handleType,500)} />
						{this.props.txt ? 
						<button id="clearColor" className="btn btn-default"><i className="fa fa-times"/> use default</button> 
						: null }
