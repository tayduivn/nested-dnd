...
const Display = ({
	...
	>
		...
		<div className="form-group">
			{!inherits ? (
				<input
					...
					required
					name="isa"
					className="input-title input-transparent"
					autoFocus={isCreate}
					placeholder="generator label"
					onChange={e => handleChange("isa", e.target.value)}
				/>
			) : (
				<span>{isa}</span>
			)}
		...
		<div className="form-group">
			<label>
				<input
					...
					checked={isUnique}
					name="isUnique"
					onChange={e => handleChange("isUnique", !isUnique)}
				/>{" "}
		...
		{!isCreate ? (
				...
				style.icon ? (
					<div>
						<select
							...
							onChange={e => handleChange(["style", "useImg"], e.target.value)}
						>
						...
						{style.useImg ? (
							...
							<IconSelect
								value={mixedToPlainType(style.icon)}
								status={{ isEnabled: true }}
								saveProperty={v => handleChange(["style", "icon"], v)}
								setPreview={() => {}}
								multi={true}
							/>
						)}
					...
					<div className="form-group">
						...
						<button
							className="btn btn-primary"
							onClick={() => handleChange(["style", "useImg"], false)}
						>
			...
			<button className="btn btn-primary" onClick={() => handleAdd(isa)}>
				Create
			</button>
		)}
	</form>
