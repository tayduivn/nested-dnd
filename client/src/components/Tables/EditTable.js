...
const Display = ({ isCreate, handleAdd = () => {}, title = "", ...rest }) => (
	<div className="form">
		{isCreate ? (
			<button className="btn btn-primary" onClick={() => handleAdd(title)}>
				Create
			</button>
		) : (
			<DisplayForm {...rest} />
	...
	handleChange = (prop, val) => {
		...
		this.setState(
			prevState => {
				...
				if (property === "rows") {
					if (!(value instanceof Array)) value = []; // temporary bandaid to fix bad data
				}
			...
			() => {
				...
				if (isValid && !this.state.isCreate) {
					...
					else this.props.handleChange(property, value);
				}
			}
		);
	};
