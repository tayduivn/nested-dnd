...
const Display = ({packs, onSelectPack, selectedPack, handleAdd}) => (
	<div>
		...
		{selectedPack ? (
			<form className="form-group mt-4 mb-3" onSubmit={(e)=>{handleAdd(e, selectedPack)}}>
				<label>Name your universe...</label>
				<input name="title" className="form-control form-control-lg" defaultValue={packs.find(p=>p._id===selectedPack).name+" Universe"} required />
				<button type="submit" className="mt-3 btn btn-lg btn-primary">Create <i className="fas fa-chevron-right" /></button>
			</form>
		) : null}
	</div>
)
		...
		return (
			<div className="main">
				<div className="container mt-5">
					<Display packs={appendBlank(this.props.data.myPacks)} selectedPack={this.state.selectedPack} onSelectPack={this.onSelectPack} onSubmit={this.onSubmit} handleAdd={this.props.handleAdd} />
				</div>
			</div>
		);
	}
...
function appendBlank(packs){
	...
	if(!packs.find((p)=>p._id === nested._id))
		packs.push(nested);

	if(!packs.find((p)=>p._id === dnd._id))
		packs.push(dnd);
