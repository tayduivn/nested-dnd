...
import { recalcFontSize, CARDS_PER_PAGE } from "../util/cardsUtil";
...
export default class Cards extends React.Component {
	...
	render() {
		...
		for (var i = 0; i < ALL_CARDS.length; i++) {
			...
			if (cards.length === CARDS_PER_PAGE || (ALL_CARDS.length && i === ALL_CARDS.length - 1)) {
				pages.push(<Page cards={cards} side="front" key={pages.length} name={name} />);
				cards = [];
			}
		}
		return (
