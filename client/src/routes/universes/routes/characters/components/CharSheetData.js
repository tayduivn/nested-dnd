	...
	<div className={`close-col col-${col}`}>
		<div className="row">
			<Item
				xs={12}
				label="Race"
				value={race.label && race.label.length ? race.label : race.name}
			/>
			<Item
				xs={8}
				label="Background"
			...
			/>
			<Item xs={4} label="Coin" value={background.startingCoin} />
			<Item
				xs={4}
				label="Speed"
				value={(body.speeds && body.speeds.walk + " ft.") || ""}
			/>
			<Item xs={4} label="Size" value={race.size.charAt(0)} />
			<Item xs={4} label="Proficient" value={profBonus} />
		</div>
		<div className="row">
...
const ProficienciesList = ({
	...
	<div className="description">
		...
		<ProfParagraph
			label="Languages"
			list={languages.join ? languages.join(", ") : languages}
		/>
		<ProfParagraph label="Armor" list={armor.join ? armor.join(", ") : armor} />
		<ProfParagraph
			label="Weapons"
			list={weapons.join ? weapons.join(", ") : weapons}
		/>
		<ProfParagraph
			label="Tools"
			list={toolsToString(tools, doubleTools, profBonus)}
		/>
	</div>
);
