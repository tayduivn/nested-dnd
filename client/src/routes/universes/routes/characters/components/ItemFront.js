...
const ItemFront = ({ item }) => (
	<div className={"card weapon" + (item.consumable ? " red" : item.uses ? " purple" : "")}>
		<div className="card-inner front">
			<h1>{item.name.trim().length ? item.name : "________________"}</h1>
...
const RollValues = ({
	...
	<div className="roll-values pin-bottom">
		...
		{uses ? (
			<div>
				<WriteIn label="Charges" dice={"❍".repeat(uses.count)} />
				<WriteIn
		...
		{attackType ? <WriteIn label="Attack" dmgType={attackType} writein={true} /> : null}
		{saveData ? <WriteIn label="Save" save={saveData.throw} writein={false} /> : null}
		...
		{damage && damage.twoHandDice ? (
			<WriteIn label="&nbsp;&nbsp;2-hand" dice={damage.twoHandDice} />
		) : null}
		{damage && damage.progression && level !== 0 && typeof damage.progression === "string" ? (
			<WriteIn
				className="higherLevelDmg"
				label={"	↑" + (uses ? "Charges" : "Levels")}
				dice={"+" + damage.progression + " per " + (uses ? "charge" : "slot level")}
				writein={false}
			/>
		) : null}
	</div>
