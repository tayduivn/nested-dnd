...
import { getDamageTypeIcon, getDamageTypeName } from "../util/cardsUtil";
import Icon from "components/Icon";

const WRITEIN = <div className="write-in">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div>;

const DamageDice = ({ dice }) => <p>{dice.replace("0.49", "1").replace("level", "2 levels")}</p>;
...
const WriteIn = ({ level, dice, progression, className, label, writein, dmgType, save }) => (
	<div className={className}>
		...
		<p className="damageType">
			<Icon name={getDamageTypeIcon(dmgType)} /> <span>{getDamageTypeName(dmgType)}</span>
		</p>
		<p>{save}</p>
	</div>
);
