...
function getSpellCard({ definition: def, alwaysPrepared }, spellcasting) {
	...
	if (!spellcasting.spells[lvl]) spellcasting.spells[lvl] = [];
	...
	return {
		...
		castTime:
			def.castingTime &&
			def.castingTime.castingTimeInterval + " " + def.castingTime.castingTimeUnit,
		properties: null,
...
function feature(
	{ definition, options = [], dynamicModifiers = [], limitedUseAbilities: limit = [], isAttack },
	c,
	clas,
	classData
) {
	...
	if (definition.name === "Thieves’ Cant") return c.proficiencies.languages.push("Thieves’ Cant");
	...
	if (definition.activationType === "Action") result.desc = "";
...
function processMod(m, fName, c, clas, classData) {
	...
	switch (m.type) {
		case "proficiency":
			...
			else if (isWeapon) c.proficiencies.weapons.push(name);
			else if (isTool) c.proficiencies.tools.push(name);
			else if (m.subType.indexOf("-saving-throws") !== -1) {
				c.abilities[m.subType.substr(0, 3)].saveProficient = true;
			} else c.proficiencies.skills.push(m.friendlySubtypeName);
			break;
		case "language":
			var lang = isHarryPotter ? harryPotterify(m.friendlySubtypeName) : m.friendlySubtypeName;
			if (!c.proficiencies.languages.includes(lang)) c.proficiencies.languages.push(lang);
			break;
		case "set":
			if (m.subType === "subclass") clas.subclasses[fName] = classData.subclass.name;
			...
			break;
		case "bonus":
			if (m.subType === "strength-score") c.abilities.str.adjust.push({ value: m.value });
			else if (m.subType === "dexterity-score") c.abilities.dex.adjust.push({ value: m.value });
			else if (m.subType === "constitution-score") c.abilities.con.adjust.push({ value: m.value });
			else if (m.subType === "intelligence-score") c.abilities.int.adjust.push({ value: m.value });
			else if (m.subType === "wisdom-score") c.abilities.wis.adjust.push({ value: m.value });
			else if (m.subType === "charisma-score") c.abilities.cha.adjust.push({ value: m.value });
			else if (m.subType === "hit-points-per-level") c.hp += m.value * c.level;
			break;
		case "expertise":
	...
	switch (name) {
		...
		case "Artificer’s Lore":
			other.push({
				name: name,
				desc:
					"Double proficiency bonus for History checks on magic items, alchemical objects, or technological devices."
			});
			break;
		case "Danger Sense":
		...
		case "Natural Explorer":
			advantages.push(
				"Initiative during your first turn attacking creatures that have not yet acted"
			);
			break;
		default:
			return;
