...
var schema = Schema({
	...
	castTime: {
		type: String,
		default: function(){
			if(this.properties && this.properties.find(p=>p.name==='Light') && !this.properties.find(p=>p.name==='Two-Handed')){
	...
	damage: {
		...
		twoHandDice: {
			...
			get: function(v){
				if(v) return v;
				var isVersatile = this.properties && this.properties.find(p=>p.name==="Versatile")
				if(!isVersatile || !this.damage.diceString) return undefined;
				var diceParts = this.damage.diceString.split('d').map(n=>parseInt(n,10));
...
schema.methods.setIcon = async function(builtpack){
	...
	if(!generator)
		generator = builtpack.getGen(this.name.toLowerCase());
	...
	if(generator.style)
		this.icon = await generator.style.makeIcon();
