var Maintainer = {
	...
	insertNew: async function(data, pack, builtpack) {
		if (!data || !pack || !builtpack) return;

		//validate isa
		if (builtpack.getGen(data.isa)) {
			var e = new Error(
				data.isa + " generator already exists in pack " + pack._id
			);
			e.status = 412;
			throw e;
		}
	...
	cleanAfterRemove: async function() {
		var builtpack = await this.model("BuiltPack")
			.findById(this.pack)
			.exec();
	...
	rename: async function(generator, pack, isaOld, builtpack) {
		...
		let result = await Promise.all([
			...
			(builtpack && builtpack.RAW_GENS) ||
				Generator.find({ pack: pack._id }).exec(),
			// set seed ----------------------------
		...
		if (builtpack) {
			...
			if (builtpack.generators[isaOld]) delete builtpack.generators[isaOld];
		...
		if (builtpack) await builtpack.save();
	}
};
...
function renameWithinGen(gen, builtpack, isaOld, isaNew) {
	...
	if (builtpack && !builtGen) {
		throw new Error(
			"Could not find build for " + gen.isa + " in pack " + gen.pack
		);
	}
