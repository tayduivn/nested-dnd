...
var Maker = {
	...
	make: async function(gen, generations, builtpack, node, ancestorData = {}){
		...
		if(!node){
			...
			let desc = (gen.desc) ? await Promise.all(
	...
	makeChild: async function(child, builtpack, generations, ancestorData = {}){
		...
		var {gen,table} = await checkTypes(child, Table, builtpack, ancestorData)
...
async function checkTypes(child, Table, builtpack, data = {}){
	...
	else if(child.type === 'embed'){ // embed
		gen = child.value;
	}
	else if(typeof child.value === 'string'){ // string
...
function cleanGen(genData, builtpack){
	...
	if(genData.chooseRandom){
		...
		for(var isa in builtpack.generators){
			if(builtpack.generators[isa].extends === genData.isa)
				choices.push(isa);
		}
		if(choices.length){
