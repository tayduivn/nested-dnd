...
module.exports = function(app) {
		...
		BuiltPack.findOrBuild(req.pack)
			.then(builtpack => {
				if (!builtpack)
					res
						.status(404)
						.json({ error: "Cannot find pack with id " + req.params.pack });
	...
	// ---------------------------------
	app.get(
		"/api/builtpacks/:pack/generators",
		MW.canViewPack,
		(req, res, next) => {
			BuiltPack.findOrBuild(req.pack)
				.then(builtpack => {
					if (typeof builtpack === "object")
						res.json(
							Object.keys(builtpack.generators).sort((a, b) =>
								a.localeCompare(b)
							)
						);
					res.status(404);
			...
			BuiltPack.findOrBuild(req.pack)
				.then(builtpack => {
					if (!req.pack.seedIsValid(newVals.seed, builtpack)) {
						return res
							.status(412)
							.json({ error: "Seed is not valid: " + newVals.seed });
					} else save();
				})
				.catch(next);
		} else save();
