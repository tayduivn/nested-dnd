...
module.exports = function(app) {
	...
	// ---------------------------------
	// TODO: Multiple packs
	app.post('/api/universes', MW.isLoggedIn, (req,res,next)=>{
		...
		MW.canViewPack(req, res, async (err)=>{
			...
			var { universe, nested } = await Universe.build(req.pack)
	...
	// Get All Universes
	// ---------------------------------
	app.get('/api/universes', MW.isLoggedIn, (req, res, next)=>{
		Universe.find({ user_id: req.user.id }).populate('pack', 'name dependencies font').then(async (universes)=>{
			Promise.all(universes.map(async (u)=>{
	...
	// Get universe
	// ---------------------------------
	app.get('/api/universes/:universe', MW.ownsUniverse, (req, res, next)=>{
	...
	// Explore universe
	// ---------------------------------
	app.get('/api/universes/:universe/explore/:index?', MW.ownsUniverse, (req, res, next)=>{
		req.universe.getNested(req.params.index, req.universe.pack).then(nested => {
	...
	// Make a new node
	// ---------------------------------
	app.post('/api/universes/:universe/explore/:index', MW.isLoggedIn, (req, res, next)=>{
		const index = parseInt(req.params.index,10)

		Universe.findById(req.params.universe).populate('pack').then(async (universe)=>{
			...
			var generator = builtpack.getGen(req.body.name);
			var generated = (generator) ? await Maker.make(generator, 0, builtpack, undefined, ancestorData) : new Nested(req.body.name);
	...
	// Edit Universe
	// ---------------------------------
	app.put('/api/universes/:universe', MW.isLoggedIn, async (req,res,next)=>{
	...
	// Delete Universe
	// ---------------------------------
	app.delete('/api/universes/:universe', MW.isLoggedIn, async (req,res,next)=>{
		Universe.findOneAndRemove({ _id: req.params.universe, user_id: req.user.id }).then(async (deleted)=>{
			if(!deleted){
				var err = new Error(`Could not find universe with id ${req.params.universe} for currently logged in user`)
				err.status = 404;
				throw err;
			}
