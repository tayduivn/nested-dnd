...
module.exports = function(app) {
	...
	// ---------------------------------
	app.get(
		"/api/universes/:universe/explore/:index?",
		MW.ownsUniverse,
		(req, res, next) => {
			req.universe
		...
		(req, res, next) => {
			...
			Universe.findById(req.params.universe)
				...
				.then(async universe => {
					...
					if (universe.user_id.toString() !== req.user.id)
				...
				if (toSave.array) {
					...
					for (var index in toSave.array) {
						...
						if (newValues.delete) universe.deleteInstance(index);
							...
							universe.array.set(
								index,
								Object.assign({}, universe.array[index].toJSON(), newValues)
							);
		...
		async (req, res, next) => {
			Universe.findOneAndRemove({
				...
				.then(async deleted => {
					if (!deleted) {
						var err = new Error(
							`Could not find universe with id ${
								req.params.universe
