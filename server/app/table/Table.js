...
tableSchema.methods.roll = async function(data) {
	if (this.returns === "fng") {
		try {
			var result = fng(
				this.rows[0] && this.rows[0].value,
				this.rows[1] && this.rows[1].value,
				1, // quantity
				(this.rows[2] !== undefined &&
					this.rows[2] !== null &&
					!isNaN(this.rows[2].value) &&
					this.rows[2].value) ||
					undefined
			)[0];
	...
	var row;
	if (this.rowWeights) row = weightedDiceChoose(this.rows);
	else if (this.tableWeight) row = weightedChoose(this.rows, this.tableWeight);
	else row = choose(this.rows);
