...
describe("Maker", () => {
	...
	describe("make()", () => {
		it("should return a node", () => {
			return Maker.make(generator, undefined, builtpack).then(node => {
				node.should.be.an
					.instanceOf(Nested)
					.and.have.property("name", "blue banana");
	...
	describe("makeChild()", () => {
		it("should return an empty array if bad input", () => {
			return Maker.makeChild()
				.should.eventually.be.an("array")
				.with.lengthOf(0);
		});

		it("should return empty array if not included", () => {
			return Maker.makeChild(generator.in[0], builtpack)
				.should.eventually.be.an("array")
				.with.lengthOf(0);
		});

		it("should return made table", () => {
			return Maker.makeChild(generator.in[1], builtpack)
				.should.eventually.be.an("array")
				.with.lengthOf(1);
		});

		it("should return made generator", () => {
			return Maker.makeChild(generator.in[2], builtpack)
				.should.eventually.be.an("array")
				.with.lengthOf(1);
		});
		...
		it("should return empty array if table is not valid", () => {
			return Maker.makeChild(generator.in[3], builtpack)
				.should.eventually.be.an("array")
				.with.lengthOf(0);
		});
	});
	...
	describe("makeMixedThing()", () => {
		...
		it("should return a string if type not set", () => {
			return Maker.makeMixedThing({ value: "test" }).should.eventually.equal(
				"test"
			);
		});

		it("should return a string if type is string", () => {
			return Maker.makeMixedThing({
				type: "string",
				value: "test"
