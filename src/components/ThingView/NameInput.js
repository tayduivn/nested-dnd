...
export default class NameInput extends React.Component {
	...
	_validate(props){
		...
		if(!props.value || props.value.trim().length === 0){
			...
			if(thingStore.exists(props.value.trim() )){
				this.validationState = "error";
				this.helpText = <span><strong>{props.value}</strong> already exists</span>;
			}
			else if(props.originalValue.trim().length) {
				this.helpText = <span>renaming <strong>{props.originalValue}</strong> to <strong>{props.value}</strong></span>
			}
			else {
				this.helpText = <span>adding new thing <strong>{props.value}</strong></span>
			}
		}
	...
	render (){
		return (
		<FormGroup className="title clearable" bsSize="large" controlId="thing-name" validationState={this.validationState}>
			<FormControl type="text" value={this.props.value} onChange={this.handleChange} />
			<HelpBlock>{this.helpText}</HelpBlock>
			<FormControl.Feedback className={this.props.value === this.props.originalValue ? "hidden": ""} onClick={this.handleClear} ><Glyphicon glyph="remove" /></FormControl.Feedback>
		</FormGroup>
		)
	}
}
